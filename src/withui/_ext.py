...
class TooltipManager:
    def __init__(self, width: int = 150, activate_after_ms: int = 800):
        with _containers.VCont(can_navigate=False, min_max_width=width, auto_resize_h=False, visible=False, free_position=pygame.Vector2()) as tooltip_cont:
            self._tooltip_cont = tooltip_cont
            self._tooltip_cont._root_index = 999999
            self._tooltip_cont._always_top = True
            self._title_label = _texts.Label(
                text="Tooltip Title", auto_resize_h=False, min_max_width=width, font_size=25)
            self._description_label = _texts.Label(
    ...
    @property
    def tooltip_container(self) -> _containers.VCont:
        return self._tooltip_cont

    @property
    def tooltip_title(self) -> _texts.Label:
        return self._title_label

    @property
    def tooltip_description(self) -> _texts.Label:
...
class TypingAnimation:
    def __init__(self, element_supporting_text: _texts.Label | _texts.Entryline | _buttons.Button, text: str, speed: int = 25, speed_comma: int = 150, speed_dot: int = 300, speed_newline: int = 400):
...
class FileDialog:
    def __init__(self,
        ...
        with _windows.Window(title=title, topleft=topleft, min_max_width=width, min_max_height=height, on_close=self._on_window_close) as win:
            self._window = win
            self._path_entry = _texts.Entryline(
            ...
            with _containers.HCont(center_elements=True, parent_anchor="right", has_outline=False, has_background=False, margin=0) as cont:
                self._cont = cont
                self._back_btn = _buttons.Button(
                    text="<-", on_click=self._on_back_click)
                self._open_btn = _buttons.Button(
                    text="Open", on_click=self._on_open_click)
                self._ok_btn = _buttons.Button(
                    text="Ok", on_click=self._on_ok_click)
                self._cancel_btn = _buttons.Button(
